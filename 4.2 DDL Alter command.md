# 📘 ALTER Command – SQL Server (SSMS)

---

## 1️⃣ What is ALTER Command?

The **`ALTER` command** is a **DDL (Data Definition Language)** statement used to **modify the definition, structure, or properties of an existing database object** in SQL Server.

<br>🔹 ALTER works **only on objects that already exist**
<br>🔹 ALTER changes **structure, not data rows**

---

## 2️⃣ Why ALTER Command is Used?

ALTER is used when:

* Business requirements change
* New columns are required
* Existing column size is insufficient
* Rules (constraints) need modification
* Stored logic needs updates
* Performance tuning is required

👉 Instead of **dropping and recreating objects (data loss risk)**, ALTER safely updates them.

---

## 3️⃣ Objects That Can Be ALTERed

| Object     | Purpose                  |
| ---------- | ------------------------ |
| DATABASE   | Change database settings |
| TABLE      | Modify table structure   |
| COLUMN     | Change column definition |
| CONSTRAINT | Add / drop rules         |
| VIEW       | Modify query logic       |
| PROCEDURE  | Update stored logic      |
| FUNCTION   | Update calculation       |
| SCHEMA     | Move objects             |
| INDEX      | Performance tuning       |

---

## 4️⃣ ALTER DATABASE

### 🔹 Purpose

Modify **database-level properties**

---

### 🔹 Syntax

```sql
ALTER DATABASE database_name
MODIFY | SET option;
```

---

### 🔹 Example 1: Rename Database

```sql
ALTER DATABASE CollegeDB
MODIFY NAME = UniversityDB;
GO
```

**What happens**

* Database name changes
* Physical data files remain unchanged
* System metadata updated

---

### 🔹 Example 2: Set Database READ ONLY

```sql
ALTER DATABASE UniversityDB
SET READ_ONLY;
GO
```

**Effect**

* INSERT / UPDATE / DELETE blocked
* SELECT allowed

---

### 🔹 Example 3: Set Database READ WRITE

```sql
ALTER DATABASE UniversityDB
SET READ_WRITE;
GO
```

---

## 5️⃣ ALTER TABLE (MOST IMPORTANT)

### 🔹 Purpose

Modify **table structure** without deleting data.

---

### 🔹 General Syntax

```sql
ALTER TABLE table_name
ADD | ALTER COLUMN | DROP COLUMN definition;
```

---

## 6️⃣ ALTER TABLE – ADD COLUMN

### 🔹 Syntax

```sql
ALTER TABLE table_name
ADD column_name datatype;
```

### 🔹 Example

```sql
ALTER TABLE Students
ADD Email VARCHAR(100);
GO
```

**What happens**

* New column added
* Existing rows get NULL values

---

## 7️⃣ ALTER TABLE – ADD MULTIPLE COLUMNS

```sql
ALTER TABLE Students
ADD Phone VARCHAR(15),
    City VARCHAR(50);
GO
```

---

## 8️⃣ ALTER TABLE – MODIFY COLUMN

### 🔹 Syntax

```sql
ALTER TABLE table_name
ALTER COLUMN column_name new_definition;
```

### 🔹 Example

```sql
ALTER TABLE Students
ALTER COLUMN StudentName VARCHAR(100);
GO
```

**What happens**

* Column size increased
* Existing data preserved
* SQL Server validates compatibility

---

## 9️⃣ ALTER TABLE – DROP COLUMN

### 🔹 Syntax

```sql
ALTER TABLE table_name
DROP COLUMN column_name;
```

### 🔹 Example

```sql
ALTER TABLE Students
DROP COLUMN City;
GO
```

⚠️ Column data permanently removed

---

## 🔟 ALTER TABLE – ADD CONSTRAINT

```sql
ALTER TABLE Students
ADD CONSTRAINT UQ_Email UNIQUE (Email);
GO
```

**What happens**

* Existing data validated
* Constraint enforced for future rows

---

## 1️⃣1️⃣ ALTER TABLE – DROP CONSTRAINT

```sql
ALTER TABLE Students
DROP CONSTRAINT UQ_Email;
GO
```

---

## 1️⃣2️⃣ ALTER VIEW

### 🔹 Purpose

Modify the **definition of an existing view**

---

### 🔹 Syntax

```sql
ALTER VIEW view_name AS
SELECT query;
```

### 🔹 Example

```sql
ALTER VIEW StudentView AS
SELECT StudentID, StudentName, Marks
FROM Students
WHERE Marks >= 60;
GO
```

---

## 1️⃣3️⃣ ALTER PROCEDURE

### 🔹 Purpose

Update stored procedure logic

---

### 🔹 Syntax

```sql
ALTER PROCEDURE procedure_name
AS
BEGIN
    SQL statements
END;
```

### 🔹 Example

```sql
ALTER PROCEDURE GetStudents
AS
BEGIN
    SELECT StudentID, StudentName
    FROM Students;
END;
GO
```

---

## 1️⃣4️⃣ ALTER FUNCTION

### 🔹 Purpose

Modify function logic

---

### 🔹 Syntax

```sql
ALTER FUNCTION function_name (parameters)
RETURNS datatype
AS
BEGIN
    RETURN value
END;
```

### 🔹 Example

```sql
ALTER FUNCTION GetResult (@Marks INT)
RETURNS VARCHAR(10)
AS
BEGIN
    RETURN
        CASE
            WHEN @Marks >= 75 THEN 'Dist'
            WHEN @Marks >= 60 THEN 'First'
            ELSE 'Pass'
        END
END;
GO
```

---

## 1️⃣5️⃣ ALTER SCHEMA

### 🔹 Purpose

Move object between schemas

---

### 🔹 Syntax

```sql
ALTER SCHEMA target_schema
TRANSFER source_schema.object;
```

### 🔹 Example

```sql
ALTER SCHEMA Academics
TRANSFER dbo.Students;
GO
```

---

## 1️⃣6️⃣ ALTER INDEX

### 🔹 Purpose

Improve query performance

---

### 🔹 Syntax

```sql
ALTER INDEX index_name | ALL
ON table_name
REBUILD | DISABLE;
```

### 🔹 Example

```sql
ALTER INDEX ALL
ON Students
REBUILD;
GO
```

---

## 1️⃣7️⃣ Important Characteristics of ALTER

| Point             | Explanation            |
| ----------------- | ---------------------- |
| Command Type      | DDL                    |
| Auto Commit       | Yes                    |
| Rollback          | ❌ Not possible         |
| Object Must Exist | Mandatory              |
| Data Impact       | Possible (DROP COLUMN) |

---

## 1️⃣8️⃣ Common Errors in ALTER

* Column already exists
* Invalid data conversion
* Constraint dependency
* Object not found
* Permission denied

---

## 1️⃣9️⃣ CREATE vs ALTER (Quick Comparison)

| CREATE                | ALTER                    |
| --------------------- | ------------------------ |
| Creates new object    | Modifies existing object |
| Used once             | Used many times          |
| Object must not exist | Object must exist        |
