# 📘 UPDATE Command – SQL Server (SSMS)

---

## 1️⃣ What is UPDATE Command?

The **`UPDATE` command** is a **DML (Data Manipulation Language)** statement used to **modify existing data (rows)** in a table.

<br>> ✔ UPDATE works on **data**, not structure
<br>> ✔ Table **must already exist**
<br>> ✔ UPDATE changes **existing rows only**

---

## 2️⃣ Why UPDATE Command is Used?

UPDATE is used when:

* Data needs correction
* Business rules change
* Values must be recalculated
* User information is modified
* Application updates records

👉 **Without UPDATE, incorrect data cannot be fixed**

---

## 3️⃣ Position of UPDATE in DML Flow

> **INSERT → UPDATE → DELETE**

INSERT adds data, UPDATE modifies it, DELETE removes it.

---

## 4️⃣ Basic UPDATE Syntax

### 🔹 Syntax

```sql
UPDATE table_name
SET column_name = value
WHERE condition;
GO
```

---

### 🔹 Example

```sql
UPDATE Students
SET Marks = 90
WHERE StudentID = 1;
GO
```

---

### 🔹 What happens?

* Only rows matching the WHERE condition are updated
* Table structure remains unchanged

⚠️ **WHERE clause is mandatory in real projects**

1️⃣1️⃣ Why WHERE is CRITICAL?

Without WHERE:

```sql
UPDATE Students SET Marks = 100;
```

❌ Updates ALL rows

With WHERE:
```sql
UPDATE Students SET Marks = 100 WHERE StudentID = 1;
```

✔ Updates only required row

---

## 5️⃣ UPDATE Multiple Columns

### 🔹 Syntax

```sql
UPDATE table_name
SET column1 = value1,
    column2 = value2
WHERE condition;
GO
```

---

### 🔹 Example

```sql
UPDATE Students
SET StudentName = 'Anam Yashwanth',
    Marks = 95
WHERE StudentID = 1;
GO
```

---

### 🔹 What happens?

* Multiple columns updated in one row
* All changes applied together

---

## 6️⃣ UPDATE Without WHERE (Dangerous)

```sql
UPDATE Students
SET Marks = 50;
GO
```

### What happens?

* **All rows** updated
* Data loss risk
* Very common interview warning

❌ **Never do this in production**

---

## 7️⃣ UPDATE Using Conditions

### Example

```sql
UPDATE Students
SET Marks = Marks + 5
WHERE Marks < 80;
GO
```

### What happens?

* Marks increased by 5 for selected rows
* Conditional update applied

---

## 8️⃣ UPDATE Using Another Table (JOIN UPDATE)

### 🔹 Syntax

```sql
UPDATE target
SET target.column = source.column
FROM target_table target
JOIN source_table source
ON condition;
GO
```

---

### 🔹 Example

```sql
UPDATE S
SET S.Marks = R.NewMarks
FROM Students S
JOIN Results R
ON S.StudentID = R.StudentID;
GO
```

### What happens?

* Data synced from another table
* Very common in real-time projects

---

### JOIN UPDATE with Additional WHERE Filter
Scenario

Update marks only for students scoring below 70.

```sql
UPDATE S
SET S.Marks = R.NewMarks
FROM Students S
JOIN Results R
ON S.StudentID = R.StudentID
WHERE S.Marks < 70;
GO
```
### Safe JOIN UPDATE (Best Practice)
Always Preview Before UPDATE
```sql
SELECT S.StudentID, S.Marks, R.NewMarks
FROM Students S
JOIN Results R
ON S.StudentID = R.StudentID
WHERE R.NewMarks > S.Marks;
```

✔ Verify rows
✔ Then run UPDATE

## 9️⃣ UPDATE with NULL Values

```sql
UPDATE Students
SET Marks = NULL
WHERE StudentID = 2;
GO
```

✔ Allowed only if column allows NULL
❌ Fails if NOT NULL constraint exists

---

## 🔟 UPDATE with TRANSACTION (SSMS Safety)

```sql
BEGIN TRAN;

UPDATE Students
SET Marks = 100
WHERE StudentID = 3;

ROLLBACK;
GO
```

### What happens?

* Change applied temporarily
* ROLLBACK cancels update
* Data remains unchanged

---

## 1️⃣1️⃣ UPDATE with COMMIT

```sql
BEGIN TRAN;

UPDATE Students
SET Marks = 88
WHERE StudentID = 3;

COMMIT;
GO
```

✔ Change saved permanently

---

## 1️⃣2️⃣ UPDATE and Constraints

UPDATE **fails** when:

* CHECK condition violated
* FOREIGN KEY violated
* UNIQUE constraint violated
* NOT NULL violated

---

## 1️⃣3️⃣ UPDATE Errors (Common)

| Error                               | Reason              |
| ----------------------------------- | ------------------- |
| Conversion failed                   | Wrong datatype      |
| Cannot insert NULL                  | NOT NULL constraint |
| Subquery returned more than one row | Bad subquery        |
| Constraint violation                | Rule broken         |

---

## 1️⃣4️⃣ UPDATE Best Practices (Real Projects)

✔ Always run SELECT before UPDATE
✔ Always use WHERE clause
✔ Use transactions for safety
✔ Avoid updating primary keys
✔ Take backup before bulk updates

---

## 1️⃣5️⃣ UPDATE vs INSERT vs DELETE

| UPDATE         | INSERT    | DELETE         |
| -------------- | --------- | -------------- |
| Modifies rows  | Adds rows | Removes rows   |
| WHERE required | No WHERE  | WHERE required |

---

## 🧠 One-Line Memory

> **UPDATE modifies existing data in a table**
