# 📘 TRUNCATE Command – SQL Server (SSMS)

---

## 1️⃣ What is TRUNCATE Command?

`TRUNCATE TABLE` is a **DDL command** used to **remove all rows from a table instantly** while **keeping the table structure intact**.

> ✔ Removes **data only**
> ✔ Keeps **table, columns, constraints**
> ❌ Cannot remove selected rows

---

## 2️⃣ Why TRUNCATE is Used in SSMS?
...
⚠️ TRUNCATE is **destructive** and **cannot be rolled back**.

---

## 3️⃣ What Can Be TRUNCATEd?
...
### 🔹 What happens internally in SQL Server
...
## 5️⃣ TRUNCATE vs DELETE (SSMS Interview Favorite)
...
## 6️⃣ TRUNCATE vs DROP (Very Important)
...
* You don’t have **ALTER permission**
...
👉 TRUNCATE needs **higher permission** than DELETE.
...
## 1️⃣1️⃣ TRUNCATE and TRANSACTIONS (Tricky)
...
👉 In SQL Server, TRUNCATE TABLE does NOT ignore transactions.
If it is executed inside an explicit transaction, it can be rolled back.

---

## 1️⃣2️⃣ TRUNCATE and TRIGGERS
...
## 1️⃣3️⃣ Real-Time Usage of TRUNCATE (Projects)
...
## 1️⃣4️⃣ Common TRUNCATE Errors (SSMS)
...
## 1️⃣5️⃣ DROP vs TRUNCATE vs DELETE (Final Interview Table)
